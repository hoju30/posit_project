...
double prefix_sum_total_double(const vector<double> &x) {
    return prefix_sum_total<double>(x);
}

template<int N, int ES>
double prefix_sum_total_posit(const vector<double> &x) {
    using p = posit<N, ES>;
    auto xp = convert_vec<p>(x);
    p res = prefix_sum_total<p>(xp);
...
int main() {
    ...
    string out_path = "data/prefix_sum_dataset.csv";
    ...
    for (int len : vec_lens) {
        for (DistType dist : dists) {
            for (int s = 0; s < num_samples_per_setting; ++s) {
                ...
                double ref64 = prefix_sum_total_double(x);
                auto xf = convert_vec<float>(x);
                float ref32f = prefix_sum_total<float>(xf);
                ...
                for (auto f : formats) {
                    ...
                    auto runner = [&](auto n_c, auto es_c) -> double {
                        return prefix_sum_total_posit<n_c, es_c>(x);
                    };
                    try {
    ...
    ofs.close();
    cout << "prefix_sum dataset written to " << out_path << "\n";
    return 0;
}
