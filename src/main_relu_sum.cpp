...
#include "kernel.hpp"  // 需要 template<typename Real> Real relu_sum(const vector<Real>&);
#include "stats.hpp"
...
// fp64 baseline：relu_sum<double>(x)，當作「近似真值」
double relu_sum_double(const vector<double> &x) {
    return relu_sum<double>(x);
}

// 專給 posit 用的 relu_sum wrapper
template<int N, int ES>
double relu_sum_posit(const vector<double> &x) {
    using p = posit<N, ES>;
    auto x_p = convert_vec<p>(x);
    p res = relu_sum<p>(x_p);
...
int main() {
    ...
    string out_path = "data/relu_sum_dataset.csv";
    ...
    for (int len : vec_lens) {
        for (DistType dist : dists) {
            for (int s = 0; s < num_samples_per_setting; ++s) {
                ...
                double ref64 = relu_sum_double(x);

                // 3) fp32 baseline
                auto x_f = convert_vec<float>(x);
                float ref32_f = relu_sum<float>(x_f);
                ...
                for (auto f : formats) {
                    ...
                    auto runner = [&](auto n_c, auto es_c) -> double {
                        return relu_sum_posit<n_c, es_c>(x);
                    };
    ...
    ofs.close();
    cout << "relu_sum dataset written to " << out_path << "\n";
    return 0;
}
