...
double sum_squares_double(const vector<double> &x) {
    return sum_squares<double>(x);
}

template<int N, int ES>
double sum_squares_posit(const vector<double> &x) {
    using p = posit<N, ES>;
    auto xp = convert_vec<p>(x);
    p res = sum_squares<p>(xp);
...
int main() {
    ...
    string out_path = "data/sum_squares_dataset.csv";
    ...
    for (int len : vec_lens) {
        for (DistType dist : dists) {
            for (int s = 0; s < num_samples_per_setting; ++s) {
                ...
                double ref64 = sum_squares_double(x);
                auto xf = convert_vec<float>(x);
                float ref32f = sum_squares<float>(xf);
                ...
                for (auto f : formats) {
                    ...
                    auto runner = [&](auto n_c, auto es_c) -> double {
                        return sum_squares_posit<n_c, es_c>(x);
                    };
    ...
    ofs.close();
    cout << "sum_squares dataset written to " << out_path << "\n";
    return 0;
}
